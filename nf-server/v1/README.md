...
Deploy the Nextflow REST executor with Docker Compose, using OCI credentials and optional bastion access for private subnets.
...
- Volumes:
  ...
  - ./tmp:/opt/bot                # Optional debug artifacts; remove if not needed
  ```
- Environment variables under `rest-executor-0` must be populated via `.env`.
- MySQL data is persisted under `./mysql-data`.
