...
def _ensure_lock_columns() -> None:
    ...
    with LOCK_COLUMNS_LOCK:
        ...
        try:
            ...
            log(f"[lock] ensure columns failed: {exc}")
...
def build_tfvars(job_id: str, row: Dict[str, Any], name: str) -> Dict[str, Any]:
    ...
    log(
        f"[executor] job {job_id} resources accelerator={accelerator} shape={shape} cpu={cpu} ram={ram} boot={boot}"
    )
    log(
...
def _run_job_sync(work_id: int, job_id: str) -> Tuple[int, str, str]:
    ...
    tfvars = build_tfvars(job_id, row, name=f"{work_id}-{job_id[:8]}")
    ...
    if cmd_script_content:
        ...
        log(f"[executor] (job={job_id}, worker={work_id}) wrote cmd_script.sh to {cmd_script_path}")
    else:
        log(f"[executor] (job={job_id}, worker={work_id}) has empty cmd_script; no cmd_script.sh written")

    log(f"[executor] (job={job_id}, worker={work_id}) cmd_script:\n{cmd_script_content}")
    ...
    apply_cmd = ["bash", str(AUTORUN_SCRIPT), str(run_dir), "apply", str(tfvars_path)]
    log(
        f"[executor] (job={job_id}, worker={work_id}) applying infra via {apply_cmd}\n{cmd_script_content}"
    )
    ...
    log(f"[executor] (job={job_id}, worker={work_id}) autorun apply {status}")
    ...
    with conn:
        with conn.cursor() as cur:
            try:
                ...
                log(f"[lock] set_lock failed for job {job_id}: {exc}")
    ...
    with conn:
        with conn.cursor() as cur:
            try:
                ...
                log(f"[lock] claim failed for status={status}: {exc}")
                return None
...
def _run_running_job(work_id: int, job_id: str) -> bool:
    ...
    if rc == 0:
        log(f"[executor] (worker={work_id}, job={job_id}) bootstrap check finished (rc={rc})")
    ...
    if rc != 0:
        log(f"[executor] (worker={work_id}, job={job_id}) bootstrap check still running (rc={rc})")
...
def _run_queued_job(work_id: int, job_id: str) -> bool:
    ...
    _set_lock(job_id, 0)
    log(f"[executor] (worker={work_id}, job={job_id}) finished status={status}")
    return True
...
def main() -> None:
    if not AUTORUN_SCRIPT.exists():
        log(f"[agent] autorun script not found at {AUTORUN_SCRIPT}; exiting")
        raise SystemExit(1)

    stop_event = threading.Event()

    def handle_signal(signum, frame):
        log(f"[agent] received signal {signum}, shutting down")
        stop_event.set()
    ...
    log(f"[agent] started {worker_count} workers")
